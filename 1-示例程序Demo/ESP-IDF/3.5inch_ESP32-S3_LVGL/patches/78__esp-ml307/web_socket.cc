...
WebSocket::~WebSocket() {
    ...
    /* Give receive thread time to notice the disconnect and exit */
...
bool WebSocket::Connect(const char* uri) {
    ...
    ESP_LOGI(TAG, "Sending WS handshake:\n%s", request.c_str());
    ...
    ESP_LOGI(TAG, "WS handshake response: %s", buffer.c_str());
    if (buffer.find("HTTP/1.1 101") == std::string::npos) {
        ESP_LOGE(TAG, "WebSocket handshake failed: %s", buffer.c_str());
        return false;
    }
